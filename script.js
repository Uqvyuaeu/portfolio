...
(function(){
  ...
  function resolveUrl(candidate){
    // Try given, then try original filename with URL encoding as a fallback
    var alt = 'White%20Paper_Hyperlink%20Submission.pdf';
    return fetch(candidate, { method:'HEAD' })
      .then(function(r){ if (r.ok) return candidate; throw 0; })
      .catch(function(){
        return fetch(alt, { method:'HEAD' })
          .then(function(r2){ if (r2.ok) return alt; throw new Error('PDF not found'); });
      });
  }
  ...
  document.querySelectorAll('.pdf-trigger').forEach(function(el){
    el.addEventListener('click', function(){
      ...
      resolveUrl(url).then(openPDF).catch(function(){
        // graceful fallback
